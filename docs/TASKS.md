...
- [ ] Código muerto: `NewPlanDialog`, `EditPlanDialog`, `PlanDetailDialog`, `DeletePlanConfirm` (`src/components/patients/dialogs/PlanDialogs.tsx`, tabla `treatment_plans`) están importados en `PatientProfile.tsx` pero ningún botón de la UI actual setea `showNewPlan`/`editPlan`/`showPlanDetail`/`deletePlan` a un valor truthy — quedaron huérfanos del viejo flujo de "Plan de tratamiento", superado por Ejercicio→Plan→Programa (`exercise_routines`/`exercise_programs`) de agosto 2026. Evaluar si borrar el import + los 4 componentes + la tabla `treatment_plans`, o si conviene reconectarlos a algún botón. Encontrado 2026-09-15 al intentar verificar en el navegador un fix de manejo de errores en ese archivo.
