-- Un plan de paciente puede haberse armado a partir de un Programa completo
-- (además de, o en vez de, un único Plan) — se guarda la trazabilidad igual
-- que ya se hace con routine_id.
ALTER TABLE exercise_plans
  ADD COLUMN program_id uuid REFERENCES exercise_programs(id) ON DELETE SET NULL;

-- Se agrega p_program_id (default NULL) a la función existente. Al llamarse
-- por nombre de parámetro desde supabase-js, es compatible con la llamada
-- actual del wizard (que todavía no manda ese parámetro).
